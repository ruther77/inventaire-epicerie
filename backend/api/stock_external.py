"""External stock management endpoints shared between legacy and new apps."""
...
router = APIRouter(tags=["stock"], include_in_schema=True)
...
@router.post("/stock/update", response_model=StockUpdateResult)
def update_stock_external(update: StockUpdate) -> StockUpdateResult:
    return _insert_stock_movement(update)


@router.post("/stock/bulk-update", response_model=BulkUpdateResponse)

...
def test_login_success(monkeypatch):
    ...
    monkeypatch.setattr('backend.api.auth.fetch_user_by_username', _fetch_user)
    monkeypatch.setattr('backend.security.fetch_user_by_username', _fetch_user)
...
def test_admin_can_list_users(monkeypatch):
    ...
    monkeypatch.setattr(
        'backend.api.users.repository_list_users',
        lambda: [
            {
                'id': 1,
                'username': 'admin',
...
def test_update_user_prevents_removing_last_admin(monkeypatch):
    ...
    monkeypatch.setattr(
        'backend.api.users.fetch_user_by_id',
        lambda user_id: {
            'id': user_id,
            'username': 'admin',
            'email': 'admin@example.test',
    ...
    monkeypatch.setattr('backend.api.users.count_active_admins', lambda exclude_user_id=None: 0)
    # Prevent database call during the test
    monkeypatch.setattr('backend.api.users.update_user_record', lambda user_id, payload: None)
